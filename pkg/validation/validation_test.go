...
func TestIsValidFlavor(t *testing.T) {
	tests := []struct {
		...
		{"valid sweet", model.FlavorSweet, true},
		{"valid bitter", model.FlavorBitter, true},
		{"valid fruity", model.FlavorFruity, true},
		{"valid spicy", model.FlavorSpicy, true},
		{"valid savory", model.FlavorSavory, true},
		{"valid sour", model.FlavorSour, true},
		{"invalid empty", "", false},
		{"invalid random", "Random", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsValidFlavor(tt.flavor); got != tt.want {
				t.Errorf("IsValidFlavor(%q) = %v, want %v", tt.flavor, got, tt.want)
			}
		})
	}
}

func TestIsValidAroma(t *testing.T) {
	tests := []struct {
		name  string
		aroma model.Aroma
		want  bool
	}{
		{"valid floral", model.AromaFloral, true},
		{"valid fruity", model.AromaFruity, true},
		{"valid malty", model.AromaMalty, true},
		{"valid spicy", model.AromaSpicy, true},
		{"valid earthy", model.AromaEarthy, true},
		{"valid citrus", model.AromaCitrus, true},
		{"valid toasty", model.AromaToasty, true},
		{"invalid empty", "", false},
		{"invalid random", "Random", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsValidAroma(tt.aroma); got != tt.want {
				t.Errorf("IsValidAroma(%q) = %v, want %v", tt.aroma, got, tt.want)
			}
		})
	}
}
...
func TestIsValidColor(t *testing.T) {
	tests := []struct {
		...
		{"valid pale", model.ColorPale, true},
		{"valid amber", model.ColorAmber, true},
		{"valid brown", model.ColorBrown, true},
		{"valid dark", model.ColorDark, true},
		{"valid black", model.ColorBlack, true},
		{"invalid empty", "", false},
		{"invalid random", "Random", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsValidColor(tt.color); got != tt.want {
...
func TestIsValidBody(t *testing.T) {
	tests := []struct {
		name string
		body model.Body
		want bool
	}{
		{"valid light", model.BodyLight, true},
		{"valid medium", model.BodyMedium, true},
		{"valid full", model.BodyFull, true},
		{"invalid empty", "", false},
		{"invalid random", "Random", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsValidBody(tt.body); got != tt.want {
				t.Errorf("IsValidBody(%q) = %v, want %v", tt.body, got, tt.want)
			}
		})
	}
}

func TestIsValidCarbonation(t *testing.T) {
	tests := []struct {
		name        string
		carbonation model.Carbonation
		want        bool
	}{
		{"valid low", model.CarbonationLow, true},
		{"valid medium", model.CarbonationMedium, true},
		{"valid high", model.CarbonationHigh, true},
		{"valid natural", model.CarbonationNatural, true},
		{"valid forced", model.CarbonationForced, true},
		{"invalid empty", "", false},
		{"invalid random", "Random", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsValidCarbonation(tt.carbonation); got != tt.want {
				t.Errorf("IsValidCarbonation(%q) = %v, want %v", tt.carbonation, got, tt.want)
			}
		})
	}
}

func TestIsValidFinish(t *testing.T) {
	tests := []struct {
		name   string
		finish model.Finish
		want   bool
	}{
		{"valid dry", model.FinishDry, true},
		{"valid sweet", model.FinishSweet, true},
		{"valid bitter", model.FinishBitter, true},
		{"valid clean", model.FinishClean, true},
		{"valid lingering", model.FinishLingering, true},
		{"invalid empty", "", false},
		{"invalid random", "Random", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsValidFinish(tt.finish); got != tt.want {
				t.Errorf("IsValidFinish(%q) = %v, want %v", tt.finish, got, tt.want)
			}
		})
	}
}
